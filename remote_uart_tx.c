...
void remote_uart_tx_char_async(char c)
{
    ...
    cyhal_uart_enable_event(
        &remote_uart_obj,                               // uart object
        (cyhal_uart_event_t)(CYHAL_UART_IRQ_TX_EMPTY),  // Events to enable/disable
        7,                                              // Priority
        true );                                         // Enable
}
...
void remote_uart_tx_interrupts_init(void)
{
    ...
    cyhal_uart_enable_event(&remote_uart_obj, (cyhal_uart_event_t)(CYHAL_UART_IRQ_TX_DONE), 7, false);
}
...
void remote_uart_event_handler_process_tx(void)
{
    ...
    else
    {
        ...
        cyhal_uart_putc(&remote_uart_obj, (uint32_t)c);
    }
}
