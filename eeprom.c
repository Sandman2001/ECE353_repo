...
void eeprom_cs_init(void)
{
	...
	rslt = cyhal_gpio_init(PIN_SPI_EEPROM_CS, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, 1);
...
{
	/* ADD CODE */
	uint8_t transmit_data[4] 	= { EEPROM_CMD_WRITE, (address >> 8), (address & 0x00FF), data };
...
{
	/* ADD CODE */
	uint8_t transmit_data[4] 	= { EEPROM_CMD_READ, (address >> 8), (address & 0x00FF), 0};
	uint8_t receive_data[4];
	cy_rslt_t rslt;
